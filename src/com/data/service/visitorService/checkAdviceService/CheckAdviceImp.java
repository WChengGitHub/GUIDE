...
public class CheckAdviceImp implements CheckAdvice {
	...
	public void setQueryDao(QueryDao queryDao) {
	 ...
	 @Override public int queryAdviceNumber() { String sql =
	 "select  count(*) from tb_advice where status=\"0\""; return
	 checkAdviceDao.query(sql); }
	...
	public Object queryTitleAndAdvice(String ADid) {
		String sql = "select Title ,Advice,Vid from tb_advice where ADid="
				+ ADid;
		tb_adviceModel tbAdviceModel = (tb_adviceModel) checkAdviceDao
				.query1(sql);
		// TODO Auto-generated method stub
	...
	public long getAdviceNumber() {
		// TODO Auto-generated method stub
		String sql = "select  count(*) from tb_advice where status=?";
		param = new LinkedList<Object>();
		param.add("0");
		Map map = new HashMap();
		try {
			list = queryDao.query(sql, param);
		} catch (Exception e) {
	...
	public tb_adviceModel getAdvice(tb_adviceModel adviceModel) {
		...
		if (list == null) {
			...
			map = (Map) list.get(0);
			tb_adviceModel adviceModel1=new tb_adviceModel();
			adviceModel1.setTitle((String) map.get("Title"));
			adviceModel1.setAdvice((String) map.get("Advice"));
			adviceModel1.setVid((String) map.get("Vid"));
			//System.out.println(map);
			//long number = (Long) map.get("count(*)");
			//System.out.println("number:" + number);
			return adviceModel1;
	...
	public void chanceAdviceStatus(tb_adviceModel adviceModel) {
		...
		param = new LinkedList<Object>();
		String Status=adviceModel.getStatus();
		if(Status.equals("s"))
			Status="2";
		...
		param.add(adviceModel.getADid());
		try {
			queryDao.update(sql, param);
		} catch (Exception e) {
			// TODO: handle exception
			System.out.println("数据库更新失败，请检查sql语句是否正确");
		}
	...
	public tb_visitorModel getAdviceEmail(tb_adviceModel adviceModel) {
		// TODO Auto-generated method stub
		String sql = "select Email from tb_visitor where Vid=?";
		param=new LinkedList<Object>();
		param.add(adviceModel.getVid());
		Map map=new HashMap();
		tb_visitorModel visitorModel = new tb_visitorModel();
		try {
			list = queryDao.query(sql, param);
		...
		if (list == null) {
			...
			map = (Map) list.get(0);
			visitorModel.setEmail((String) map.get("Email"));
			return visitorModel;
		}
